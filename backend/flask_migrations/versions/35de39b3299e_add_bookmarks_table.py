...
def upgrade():
    ...
    with op.batch_alter_table('institutions', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f('uq_institutions_slug'), type_='unique')

    with op.batch_alter_table('organizations', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f('uq_organizations_slug'), type_='unique')
        batch_op.drop_constraint(batch_op.f('organizations_institution_id_fkey'), type_='foreignkey')
        batch_op.create_foreign_key(None, 'institutions', ['institution_id'], ['id'], ondelete='SET NULL')
...
def downgrade():
    ...
    with op.batch_alter_table('organizations', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key(batch_op.f('organizations_institution_id_fkey'), 'institutions', ['institution_id'], ['id'], ondelete='CASCADE')
        batch_op.create_unique_constraint(batch_op.f('uq_organizations_slug'), ['slug'], postgresql_nulls_not_distinct=False)

    with op.batch_alter_table('institutions', schema=None) as batch_op:
        batch_op.create_unique_constraint(batch_op.f('uq_institutions_slug'), ['slug'], postgresql_nulls_not_distinct=False)
